...
private class FlutterBindingsPigeonCodecReader: FlutterStandardReader {
    override func readValue(ofType type: UInt8) -> Any? {
        ...
        case 129:
            ...
            return nil
        case 130:
            return AlarmSettingsWire.fromList(readValue() as! [Any?])
        case 131:
            return VolumeSettingsWire.fromList(readValue() as! [Any?])
        case 132:
            return VolumeFadeStepWire.fromList(readValue() as! [Any?])
        case 133:
            return NotificationSettingsWire.fromList(readValue() as! [Any?])
        default:
...
class AlarmTriggerApi: AlarmTriggerApiProtocol {
    ...
    func alarmRang(alarmId alarmIdArg: Int64, completion: @escaping (Result<Void, PigeonError>) -> Void) {
        let channelName = "dev.flutter.pigeon.alarm.AlarmTriggerApi.alarmRang\(messageChannelSuffix)"
    ...
    func alarmStopped(alarmId alarmIdArg: Int64, completion: @escaping (Result<Void, PigeonError>) -> Void) {
        let channelName = "dev.flutter.pigeon.alarm.AlarmTriggerApi.alarmStopped\(messageChannelSuffix)"
        let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
        channel.sendMessage([alarmIdArg] as [Any?]) { response in
            guard let listResponse = response as? [Any?] else {
                completion(.failure(createConnectionError(withChannelName: channelName)))
